...
nextToLeft(A,B,Ls):-  % a exactly to the right of b
    append(_,[A,B|_],Ls).

nextTo(A,B,Ls):-   % b next to a
    append(_,[B,A|_],Ls).
nextTo(A,B,Ls):-
    append(_,[A,B|_],Ls).
...
atAnEnd(A,Ls):-    % check if the first one or the last one
    Ls=[A|_].
atAnEnd(A,Ls):-
    Ls=[_,_,_,_,A].
...
somewhereBetween(A,B,C,Ls):-  % is between a and c if a is somewhere left then b and b somewhere left than c
