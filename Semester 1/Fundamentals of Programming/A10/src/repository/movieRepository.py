...
class MovieRepository:
    def __init__(self, list_of_movie=None):
        if list_of_movie is None:
            list_of_movie = []
        if list_of_movie is IterableData:
            self._list_of_movie = list_of_movie
        else:
            self._list_of_movie = IterableData(list_of_movie)
    ...
    def remove_movie(self, movie_Id):
        ...
        self._list_of_movie.remove(position)
    ...
    def find_movie_in_movies_list(self, movie_id):
        ...
        """
        elements = filter_method(self._list_of_movie, lambda element: int(element.get_movieId) == int(movie_id))
        if len(elements) == 0:
            return -1
        else:
            return elements[0].get_movieId-1
        # index = 0
        # for movie in self._list_of_movie:
    ...
    def determine_the_list_for_the_most_rented_movies(self, list_of_the_sorted_rented_days):
        ...
        new_list_with_movies = sorting_algorithm(new_list_with_the_keys_movies_id, lambda first, second: first > second)
        for element in new_list_with_movies:
...
class FileTextMovieRepository(MovieRepository):
    ...
    def __write_all_to_file(self):
        with open(self.__file_path, 'w') as f:
            for movie in self._list_of_movie:
                f.write(self.__write_function(movie) + '\n')
...
#
# class MoviesRepoJson(MovieRepository):
...
#             self.add_student(Movie(obj["sid"], obj["name"]))
#         file.close()
#
#     def _save_file(self):
#         """
