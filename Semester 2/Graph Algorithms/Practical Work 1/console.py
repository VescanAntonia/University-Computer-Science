from Graph import Graph, read_a_graph_from_the_file, write_graph_to_file
...
class UI:
    ...
    def start(self):
        while True:
            try:
                self.show_menu(self)
                user_option = input(" No graph has been selected so the actual graph is currently empty. Enter "
                                    "your option: ")
                if user_option == '1':
