...
public class PolynomialOperation {
    ...
    /**
     * Simple sequential multiplication operation over 2 polynomials.
    ...
    public static Polynomial multiplicationParallelizedForm(Polynomial p1, Polynomial p2, int nrOfThreads) throws
        ...
        int step = result.getLength() / nrOfThreads;   //devide the nr of tasks
        if (step == 0) {
            step = 1;
        }
        int end;
    ...
    public static Polynomial multiplicationKaratsubaParallelizedForm(Polynomial p1, Polynomial p2, int currentDepth)
        ...
        //if it exceeded depth 4 falls back to multiplicationKaratsubaSeqForm
        // to prevent excessive parallelization
        if (currentDepth > 4) {
            return multiplicationKaratsubaSequentialForm(p1, p2);
        }

        //the same if either degree<2
        if (p1.getDegree() < 2 || p2.getDegree() < 2) {
            return multiplicationKaratsubaSequentialForm(p1, p2);
        }
        ...
        //create a thread pool that automatically adjust the size
        ThreadPoolExecutor executor = (ThreadPoolExecutor) Executors.newCachedThreadPool();
        //three tasks for each case:lower halves, sum of halves and higher halves
