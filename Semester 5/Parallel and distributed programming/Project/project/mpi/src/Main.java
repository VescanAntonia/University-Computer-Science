...
public class Main {
    ...
    private static void searchMaster(Matrix root) {
        ...
        q.add(root);
        //adds states to the queue until there are enough states to distribute to all worker processes
        while (q.size() + q.peek().generateMoves().size() - 1 <= workers) {
            Matrix currentBoard = q.poll();
            for (Matrix neighbour : currentBoard.generateMoves()) {
                q.add(neighbour);
            }
        }
        ...
        while (!found) {
            ...
            for (int i = 0; i < q.size(); i++) {
                ...
                Matrix currentBoard = temp.poll();
                MPI.COMM_WORLD.Send(new boolean[]{false}, 0, 1, MPI.BOOLEAN, i + 1, 0);
                MPI.COMM_WORLD.Send(new Object[]{currentBoard}, 0, 1, MPI.OBJECT, i + 1, 0);
    ...
    private static void searchWorker() {
        //perform the tasks until told to shut down
        while (true) {
            Object[] matrix = new Object[1];
            int[] bound = new int[1];
            boolean[] end = new boolean[1]; //flag whether it should shut down
    ...
    public static Pair<Integer, Matrix> search(Matrix current, int numSteps, int bound) {
        //recursive function that implements the A* search algorithm
        ...
        for (Matrix next : current.generateMoves()) {
            ...
            //keep track of minimum cost and corresponding solution
            if (t < minCost) {
                minCost = t;
                solution = result.getSecond();
            }
        }
        return new Pair<>(minCost, solution);
    }
