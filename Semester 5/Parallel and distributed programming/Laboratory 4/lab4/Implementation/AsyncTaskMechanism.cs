...
{
    internal class AsyncTaskMechanism
    {
        //implementation of an HTTP client using the Task-based Asynchronous Pattern
        private static List<string> HOSTS;
        private static List<Task> TASKS; //A list to store Task objects for concurrent processing

        public static void Run(List<string> hostnames)
        {
            //Initiates the execution of the HTTP client by providing a list of hostnames
            HOSTS = hostnames;
            TASKS = new List<Task>();
        ...
        {
            //Called for each hostname in the list and initiates the HTTP client for that hostname
            var id = (int)idObject;
        ...
        private static async void StartClient(string host, int id)
        {
            ...
            //await - provides a non-blocking way to start a task, then continue execution when that task completes
