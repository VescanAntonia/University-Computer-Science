...
public class Matrix {
    ...
    public static int getProductCell(Matrix m1, Matrix m2, int a, int b) {
        //returns the product of a cell
        int res = 0;
        for (int i = 0; i < m1.m; ++i)
            res += m1.getElement(a, i) * m2.getElement(i, b);
        return res;
    }
    ...
    public static Matrix multiplyMatrices(Matrix m1, Matrix m2) {
        ...
        for (int i = 0; i < m1.n; ++i)
            for (int j = 0; j < m2.m; ++j)
                product[i][j] = getProductCell(m1, m2, i, j);
